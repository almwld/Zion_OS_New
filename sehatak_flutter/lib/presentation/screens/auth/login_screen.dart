...
class _LoginScreenState extends State<LoginScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _checkBiometric() async {
    ...
    if (available) {
      ...
      setState(() {
        _hasBiometric = true;
        _bioName = _bio.getBiometricName(types);
      });
    }
  }
  ...
  Future<void> _loginWithBiometric() async {
    ...
    if (ok && mounted) {
      context.read<AuthBloc>().add(LoginWithEmail(email: 'bio@sehatak.com', password: 'bio'));
    }
  }
        ...
        body: Container(
          ...
          child: SafeArea(child: Center(child: SingleChildScrollView(padding: const EdgeInsets.all(20), child: Column(children: [
            ...
            if (_hasBiometric) ...[
              GestureDetector(
                ...
                child: Container(
                  ...
                  child: Icon(Icons.fingerprint, color: Colors.white, size: 35),
                ),
              ),
              const SizedBox(height: 6),
              Text('تسجيل الدخول بـ $_bioName', style: const TextStyle(color: Colors.white70, fontSize: 12, fontFamily: 'Cairo')),
            ...
            SizedBox(width: double.infinity, height: 48, child: OutlinedButton(
              ...
              child: const Row(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(Icons.g_mobiledata, color: Colors.red, size: 24), SizedBox(width: 10), Text('المتابعة بـ Google', style: TextStyle(fontSize: 15, color: Colors.white))]),
            )),
