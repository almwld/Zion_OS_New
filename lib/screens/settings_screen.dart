...
import 'operation_mode_screen.dart';

class SettingsScreen extends StatelessWidget {
  const SettingsScreen({super.key});
  static const _themeColors = <Color>[Color(0xFF00BCD4), Colors.cyan, Colors.green, Colors.blue, Colors.purple, Colors.orange, Colors.pink, Colors.teal, Colors.indigo];
      ...
      return Scaffold(
        ...
        body: SafeArea(child: ListView(padding: const EdgeInsets.only(bottom: 32), children: [
          ...
          _card(surface, children: [
            ListTile(contentPadding: EdgeInsets.zero, leading: Icon(Icons.hub_outlined, color: theme.primaryColor), title: Text('System Capabilities', style: TextStyle(color: foreground, fontWeight: FontWeight.w600)), subtitle: Text('Live runtime availability, permissions and configuration state', style: TextStyle(color: secondary)), trailing: Icon(Icons.chevron_right, color: secondary), onTap: () => Navigator.of(context).push(MaterialPageRoute(builder: (_) => const SystemCapabilitiesScreen()))),
            const Divider(height: 1),
            ListTile(contentPadding: EdgeInsets.zero, leading: Icon(Icons.tune, color: theme.primaryColor), title: Text('Operation Mode', style: TextStyle(color: foreground, fontWeight: FontWeight.w600)), subtitle: Text('Adaptive presentation and workflow mode', style: TextStyle(color: secondary)), trailing: Icon(Icons.chevron_right, color: secondary), onTap: () => Navigator.of(context).push(MaterialPageRoute(builder: (_) => const OperationModeScreen()))),
          ]),
          ...
          _card(surface, children: [
            ...
            const Divider(height: 1), _colorSelector(theme, foreground, secondary), const Divider(height: 1),
          ...
          _card(surface, children: [ListTile(contentPadding: EdgeInsets.zero, leading: Icon(Icons.translate, color: theme.primaryColor), title: Text('language'.tr()), subtitle: Text(context.locale.languageCode == 'ar' ? 'العربية' : 'English', style: TextStyle(color: secondary)), trailing: Icon(Icons.chevron_right, color: secondary), onTap: () => _showLanguageDialog(context))]),
  ...
  Future<void> _showChangePinDialog(BuildContext context, ThemeProvider theme) async { final oldController = TextEditingController(); final newController = TextEditingController(); final formKey = GlobalKey<FormState>(); await showDialog<void>(context: context, builder: (dialogContext) => AlertDialog(title: Text('change_pin'.tr()), content: Form(key: formKey, child: Column(mainAxisSize: MainAxisSize.min, children: [TextFormField(controller: oldController, obscureText: true, keyboardType: TextInputType.number, maxLength: 4, decoration: const InputDecoration(labelText: 'Current PIN'), validator: (v) => v != null && v.length == 4 ? null : 'Enter 4 digits'), TextFormField(controller: newController, obscureText: true, keyboardType: TextInputType.number, maxLength: 4, decoration: const InputDecoration(labelText: 'New PIN'), validator: (v) => v != null && RegExp(r'^\d{4}$').hasMatch(v) ? null : 'Enter 4 digits')])), actions: [TextButton(onPressed: () => Navigator.pop(dialogContext), child: Text('cancel'.tr())), FilledButton(onPressed: () async { if (!formKey.currentState!.validate()) return; final ok = await theme.changePin(oldController.text, newController.text); if (!dialogContext.mounted) return; if (ok) { Navigator.pop(dialogContext); ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('PIN changed successfully'), backgroundColor: theme.primaryColor)); } else { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Invalid PIN'), backgroundColor: Colors.red)); } }, child: Text('save'.tr()))]); oldController.dispose(); newController.dispose(); }
  Future<void> _showLanguageDialog(BuildContext context) async { await showDialog<void>(context: context, builder: (dialogContext) => AlertDialog(title: Text('language'.tr()), content: Column(mainAxisSize: MainAxisSize.min, children: [ListTile(title: const Text('العربية'), onTap: () async { await context.setLocale(const Locale('ar')); if (dialogContext.mounted) Navigator.pop(dialogContext); }), ListTile(title: const Text('English'), onTap: () async { await context.setLocale(const Locale('en')); if (dialogContext.mounted) Navigator.pop(dialogContext); })]))); }
}
