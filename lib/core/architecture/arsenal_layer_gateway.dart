...
class ArsenalLayerGateway {
  ...
  static const blocked = <String>{
    'wifi.wps.bruteforce', 'wifi.deauth.emit', 'wifi.evil_twin.capture',
    'wifi.credential_capture', 'credential.theft', 'password.cracking',
    'handshake.cracking', 'pmkid.cracking', 'sql.exploitation',
    'sql.data_extraction', 'metasploit.execute', 'hydra.execute',
  };
  ...
  Future<Map<String, dynamic>> dispatch(
    ...
    if (!isAllowed(normalized)) {
      return {'status': 'BLOCKED', 'module': module, 'action': normalized, 'reason': 'defensive security policy'};
    }
    try {
  ...
  Map<String, dynamic> zionWebInspect(List<int> packetBytes) => {
    'status': 'REAL',
    'module': 'ZionWeb',
    'packet': packets.inspect(Uint8List.fromList(packetBytes)),
  };
